...
const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { q: "Sing a psalm from the Metrical Psalter", a: "Open a Psalm with the Metrical Psalter pane beside it and the psalm appears in stanzas, broken into the lines it is sung in, with its metre named at the top right. A psalm is sung to any tune in its own metre, so the tune list offers every tune that fits — Old 100th and Tallis' Canon for a Long Metre psalm, St. Anne and St. Flavian for a Common Metre one — and remembers the tune you chose for that metre. Play sounds the melody and lights each word as it is sung; the slider sets the speed, and the arrows raise or lower the pitch to something your congregation can reach. “Sing through” goes stanza by stanza without stopping, and the download button saves the tune as a MIDI file." },
  ...
  },
  {
    ...
    items: [
      ...
      { q: "A note on 1, 2 and 3 John", a: "This edition abbreviates all four Johannine books “Joh”, and 259 of its references to the three epistles were tagged as the Gospel — the article on Gaius, who 3 John is written to, pointed at John 1. Those have been put right, so the links land where the text says and the epistles now answer with their own articles." },
      ...
      { q: "What the encyclopedia says about the chapter you are reading", a: "Add an “Encyclopedia for passage” pane from the icons on the right edge, beside a Bible pane. It lists the articles that discuss the open chapter and which verses each one cites — open Genesis 14 and you get Amraphel, Tidal, Chedorlaomer, Melchizedek, the King’s Vale. Select a verse and the articles on that verse move to the top. It ranks by how much of an article is about this chapter, so the one written about your passage comes before the one that mentions it in passing." },
      { q: "Finding your way through a long article", a: "Articles with several sections get a Contents list built from their own headings — beside the text when the pane is wide, folded into a box at the top when it is narrow. Click any section to jump to it. Some of the longest articles print their outline as ordinary paragraphs rather than headings; those are picked up too, so “Jesus Christ” has all 183 of its sections listed." },
    ],
  },
  {
    title: "Atlas",
