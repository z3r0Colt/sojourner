...
mod tests {
    ...
    fn migrating_an_older_database_rebuilds_its_search_indexes_from_the_text() {
        ...
        assert_eq!(search::search_notes(&conn, &search::SearchOptions::default().parse("oath"), 10).unwrap().len(), 1);
        assert_eq!(search::search_notes(&conn, &search::SearchOptions::default().parse("without"), 10).unwrap().len(), 1, "a word formatting had split");
        assert_eq!(search::search_notes(&conn, &search::SearchOptions::default().parse("Nicodemus"), 10).unwrap().len(), 1, "chapter notes too");
        assert!(search::search_notes(&conn, &search::SearchOptions::default().parse("strong"), 10).unwrap().is_empty(), "no longer by a tag name");
        assert!(search::search_notes(&conn, &search::SearchOptions::default().parse("href"), 10).unwrap().is_empty(), "nor by an attribute");
        let hit = &search::search_notes(&conn, &search::SearchOptions::default().parse("sworn"), 10).unwrap()[0];
    ...
    fn search_indexes_hold_the_words_and_not_the_markup() {
        ...
        assert_eq!(search::search_notes(&conn, &search::SearchOptions::default().parse("oath"), 10).unwrap().len(), 1, "found by its words");
        assert!(search::search_notes(&conn, &search::SearchOptions::default().parse("strong"), 10).unwrap().is_empty(), "not by a tag name");
        assert!(search::search_notes(&conn, &search::SearchOptions::default().parse("href"), 10).unwrap().is_empty(), "not by an attribute");
        let hit = &search::search_notes(&conn, &search::SearchOptions::default().parse("sworn"), 10).unwrap()[0];
        assert!(!hit.snippet.contains('<'), "the snippet is prose, not markup: {}", hit.snippet);
        assert!(hit.snippet.contains("\u{2}sworn\u{3}"), "the match is still marked: {}", hit.snippet);
        ...
        assert_eq!(search::search_notes(&conn, &search::SearchOptions::default().parse("without"), 10).unwrap().len(), 1, "reindexed, and the word is whole");
        assert!(search::search_notes(&conn, &search::SearchOptions::default().parse("oath"), 10).unwrap().is_empty(), "the old text is gone");
    ...
    fn a_chapter_note_still_surfaces_when_passage_notes_fill_the_limit() {
        ...
        let results = search::search_notes(&conn, &search::SearchOptions::default().parse("perseverance"), limit).unwrap();
        ...
        let few = search::search_notes(&conn, &search::SearchOptions::default().parse("perseverance"), 5).unwrap();
        assert_eq!(few.len(), 5);
    ...
    fn soft_deleted_note_is_hidden_everywhere_until_restored() {
        ...
        assert_eq!(search::search_notes(&conn, &search::SearchOptions::default().parse("loved"), 10).unwrap().len(), 1);
        ...
        assert!(search::search_notes(&conn, &search::SearchOptions::default().parse("loved"), 10).unwrap().is_empty(), "search");
